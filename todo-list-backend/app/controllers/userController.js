...
let resetPassword = (req, res) => {
	...
	//let userId = re.params.userId;
    let password = passwordLib.hashpassword(req.body.password);
    console.log(password);
    UserModel.updateOne({ 'userId': req.body.userId }, {$set:{'password':password}}).exec((err, result) => {
        if (err) {
            console.log(err)
...
let getAuthToken = (req, res) => {
    ...
    findUser(req,res)
        ...
        .then(saveToken)
        .then((resolve) => {
            let apiResponse = response.generate(false, 'Auth Token Generated', 200, resolve)
            res.status(200)
            res.send(apiResponse)
        })
        .catch((err) => {
...
let createList = (req, res) => {
    ...
    let validateUserInput = () => {
        return new Promise((resolve, reject) => {
            if (req.body.title) {
                ...
                logger.error('Field Missing During List Creation', 'userController: createList()', 5)
                let apiResponse = response.generate(true, 'Title is Required', 400, null)
                reject(apiResponse);
            }
        })
    }// end validate user input
    ...
    let checkList = () => {
		return new Promise((resolve,reject) => {
            TodolistModel.findOne( {userId:req.body.userId} )
            .exec((err,retrievedUserDetails) => {
                if (err) {
                    logger.error(err.message, 'userController: createUser', 10)
                    ...
                    reject(apiResponse)
                        ...
                        let newList = new TodolistModel({
                            ...
                            lists: {listId:shortid.generate(),listOpen:true,title:req.body.title},
						...
						let lId = shortid.generate();
						TodolistModel.update({userId:req.body.userId},{ $push:{ lists:{ listId:lId,listOpen:true,title:req.body.title} }})
						.exec((err,result) =>{
							if (err) {
									console.log(err)
									logger.error(err.message, 'list Controller:editUser', 10)
									let apiResponse = response.generate(true, 'Failed To edit list details', 500, null)
									res.send(apiResponse)
								} else if (check.isEmpty(result)) {
									logger.info('No list Found', 'User Controller: editUser')
									let apiResponse = response.generate(true, 'No User Found', 404, null)
									res.send(apiResponse)
								} else {
									let apiResponse = response.generate(false, 'List Added', 200, result)
									res.send(apiResponse)
								}
						});
					}
...
/*
    let createUser = () => {
        return new Promise((resolve, reject) => {
            TodolistModel.findOne({ lists: { $elemMatch: { title: req.body.title } } })
                .exec((err, retrievedUserDetails) => {
                    if (err) {
                        logger.error(err.message, 'userController: createUser', 10)
                        let apiResponse = response.generate(true, 'Failed To Create List', 500, null)
                        ...
                        logger.error('List Cannot Be Created.List Already Present with same title', 'userController: createUser', 4)
                        let apiResponse = response.generate(true, 'List Already Present With this Title', 403, null)
                        reject(apiResponse)
                    }
                })
        })
...
/*let addItem = () =>{
	...
	let additems = () =>{
		return new Promise((resolve, reject) => {
			if(req.body.listId){
                TodolistModel.findOne({ lists: { $elemMatch: { listId: req.body.listId } } })
                .exec((err,retrievedUserDetails) => {
                    
                })
			}
			else{
				logger.error('ListId is required', 'userController: additems()', 5)
                let apiResponse = response.generate(true, 'One or More Parameter(s) is missing', 400, null)
                reject(apiResponse);
			}
